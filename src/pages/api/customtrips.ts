...
export default async function handler(req:NextApiRequest,res:NextApiResponse) {
    if(req.method=="POST"){
        const {name,phone,email,triplocation,passengers,startDate,day,night} = req.body
        if(!name || !phone || !email || !triplocation || !passengers || !startDate || !day || !night){
            res.status(400).send("Some data might be missing")
        }

        const {data, error}= await supabase.from('customtrips').insert({name,triplocation,phone,email,passengers,startDate,day,night})
        if(error){
        ...
        try {
            await transporter.sendMail({
                ...
                html: `<h1>You Recieved A Trip Request from Lambda Adventures Website</h1><p><strong>Name:</strong> ${name}</p><p><strong>Phone:</strong> ${phone}</p><p><strong>Email:</strong> ${email}</p><p><strong>Message:</strong> ${triplocation}</p>`,
            });
